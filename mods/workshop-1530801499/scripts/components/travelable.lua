...
function Travelable:ListDestination(traveller)
    ...
    for k, v in pairs(dests) do
        if v.components.travelable and
            not (v.components.travelable.ownership and v:HasTag(ownershiptag) and
                traveller.userid ~= nil and
                not v:HasTag("uid_" .. traveller.userid)) then
            ...
            self.traveltask = self.inst:DoTaskInTime(8, function()
                ...
                for k, who in pairs(self.travellers) do
                    ...
                    elseif not (who:IsValid() and self.inst:IsValid() and
                        ...
                        print(
                            "player/sign is invalid, or they are far from each other.")
                        ...
                        if inventory then
                            for ki, vi in pairs(inventory.itemslots) do
                                if vi.components.leader and
                                    ...
                                    for kif, vif in
                                        pairs(vi.components.leader.followers) do
                                        if kif.Physics ~= nil then
                                            kif.Physics:Teleport(xf, 0, zf + 1)
                                        else
                                            kif.Transform:SetPosition(xf, 0,
                                                                      zf + 1)
                                        end
                                    end
                                end
                        ...
                        if container then
                            for kb, vb in pairs(container.slots) do
                                if vb.components.leader and
                                    ...
                                    for kbf, vbf in
                                        pairs(vb.components.leader.followers) do
                                        if kbf.Physics ~= nil then
                                            kbf.Physics:Teleport(xf, 0, zf - 1)
                                        else
                                            kbf.Transform:SetPosition(xf, 0,
                                                                      zf - 1)
                                        end
                                    end
                                end
...
function Travelable:EndTravel()
    if self.traveller ~= nil then
        ...
        self.inst:RemoveEventCallback("ms_closepopups", self.onclosepopups,
                                      self.traveller)
        self.inst:RemoveEventCallback("onremove", self.onclosepopups,
                                      self.traveller)
