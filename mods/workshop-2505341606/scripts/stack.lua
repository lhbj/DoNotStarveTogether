...
-- 物品自动堆叠检测范围
local SEE_ITEM_STACK_DIST = GetModConfigData("stack_scanning_range")
local stack_fireflies = GetModConfigData("stack_fireflies")
local PI = 3.14159
...
if IsServer and auto_stack then
    ...
    local function CanStackInst(inst)
        return inst:IsValid() and inst.components.stackable and not inst.components.stackable:IsFull() and
                inst.components.health == nil and inst.components.mine == nil -- 带血量的（小动物）和陷阱（有的人会装那类 mod）不堆叠
    end
        ...
        for _, objBase in pairs(ents) do
            if CanStackInst(objBase) then
                for _, obj in pairs(ents) do
                    if obj:IsValid() then
						if stack_fireflies then
							AnimPut(objBase, obj)
						else
							if obj.prefab ~= "fireflies" then
							AnimPut(objBase, obj)
							end
						end
                    end
